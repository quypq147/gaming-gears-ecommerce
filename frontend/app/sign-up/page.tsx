        ...
        <Card className="w-full max-w-md bg-opacity-10 backdrop-blur-lg border border-gray-700 shadow-xl rounded-xl text-white">
          ...
          <CardContent>
            <motion.form onSubmit={handleSignUp} className="space-y-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <Input type="text" placeholder="Tên người dùng" className="bg-gray-800 border-gray-600 focus:ring-2 focus:ring-blue-500 text-white" value={name} onChange={(e) => setName(e.target.value)} required />
              ...
              <div className="relative">
                <Input type={passwordVisible ? "text" : "password"} placeholder="Mật khẩu (tối thiểu 8 ký tự)" className="bg-gray-800 border-gray-600 focus:ring-2 focus:ring-blue-500 text-white pr-10" value={password} onChange={(e) => setPassword(e.target.value)} required />
              ...
              <div className="relative">
                <Input type={confirmPasswordVisible ? "text" : "password"} placeholder="Nhập lại mật khẩu" className={`bg-gray-800 border-gray-600 focus:ring-2 focus:ring-blue-500 text-white pr-10 ${confirmPassword && confirmPassword !== password ? "border-red-500" : ""}`} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
              ...
              <div className="flex items-center space-x-2">
                ...
                <label htmlFor="terms" className="text-sm text-gray-400">
                  Tôi chấp nhận <Link href="/terms" className="text-blue-500 hover:text-blue-400">Điều khoản & Điều kiện</Link>
                </label>
              </div>
              <ReCAPTCHA sitekey={process.env.NEXT_PUBLIC_RECAPTCHA_SITE_KEY || ""} onChange={(token) => setRecaptchaToken(token)} />
              <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 transition" disabled={loading}>{loading ? "Đang tạo tài khoản..." : "Đăng ký"}</Button>
            </motion.form>
          </CardContent>
        </Card>
      </motion.div>
